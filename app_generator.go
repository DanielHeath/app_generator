/* ./app_generator sets up a new webapp with several key builtins.

Get started

go get github.com/DanielHeath/app_generator

go build github.com/DanielHeath/app_generator

Usage

'./app_generator -name APPNAME'
*/
package main
...
func init() {
	var err error
	flag.StringVar(&config.Name, "name", "", "Application Name")
	flag.StringVar(&config.TargetPath, "path", "", "Path to generate application (default == name)")
	flag.StringVar(&config.TemplateDir, "templateDir", "./src/github.com/DanielHeath/app_generator/templates", "Path to application templates")
	...
	if err != nil {
		panic("Failed to parse templates (see `-templateDir` flag) in '" + config.TemplateDir + "': " + err.Error())
	}
...
func main() {
	...
	md("bin")
	putFile("bin/.gitignore", []byte(GitIgnore))
	md("pkg")
	putFile("pkg/.gitignore", []byte(GitIgnore))
	md("public")
	putFile("public/.gitignore", []byte(GitIgnore))
	...
	md("assets")
	putFile("assets/application.css", []byte(CssFile))
	putFile("assets/application.js", []byte(JsFile))
